...
**推奨運用:** git clone した x-tweetcraft フォルダ内で `claude --plugin-dir .` を実行し、**プラグインディレクトリ内でユーザーデータも管理** する（ユーザーデータは `.gitignore` で除外済み）。

**ユーザー作業ディレクトリ（＝プラグインディレクトリ）に生成される構造:**
...
### 作業ディレクトリとプラグインディレクトリの関係

**推奨モデル（シンプル）:** プラグインディレクトリ内で直接作業する。
- git clone したフォルダに入って `claude --plugin-dir .` を実行
- ユーザーデータ（`brand-voice.md`, `personal-info/`）はそのフォルダ内に作成される
- `.gitignore` でユーザーデータは除外されているため、プラグインリポジトリが汚れない
- 全てが一箇所にまとまっているので「どこにデータがあるか」で迷わない

**別ディレクトリで運用する場合（上級者向け）:** プラグインとユーザーデータを分離。
- VITAL Z便宜: `ceo/x-automation/` をユーザーデータの保存先にする
- ユーザー作業フォルダから `claude --plugin-dir /path/to/x-tweetcraft` で起動
- データ探索順序は各スキルで統一: `./` → `./ceo/x-automation/` → `~/`
