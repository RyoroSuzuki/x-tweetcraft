...
**Once extension is ready**, present the extraction prompt. Read `${CLAUDE_PLUGIN_ROOT}/data/browser-extraction-prompts.md` and use **Prompt 1 (自分のツイート抽出)**. Tell the user:

> Xのあなたのプロフィールページを開いた状態で、Claude拡張に以下のプロンプトを貼ってください（プラグイン同梱の共通プロンプトを使用）：
>
> [Prompt 1 from browser-extraction-prompts.md をここに展開して提示]
>
